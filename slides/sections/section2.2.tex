...
\begin{frame}
  ...
  \begin{block}{概述}
    Ion Torrent（Ion semiconductor sequencing）是一种基于半导体芯片的新一代革命性测序技术，通过检测H+信号的变化来获得序列碱基信息。该技术使用了一种布满小孔的高密度半导体芯片，一个小孔就是一个测序反应池。当DNA聚合酶把核苷酸聚合到延伸中的DNA链上时，会释放出一个氢离子，反应池中的pH发生改变，位于池下的离子感受器感受到H+离子信号，H+离子信号再直接转化为数字信号，从而读出DNA序列。
  \end{block}
\end{frame}
...
\begin{frame}
  \frametitle{基因组学 | 测序 | 第三代 | 纳米孔单分子测序}
  \begin{block}{概述}
